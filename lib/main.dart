...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> requestPermissions() async {
    ...
    if (statuses[Permission.contacts]!.isGranted &&
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Permissions not granted. Please allow all permissions.'),
        ),
      );
    }
  }
  ...
  Future<void> loadGalleryImages(String uid) async {
    ...
    try {
      ...
      final List<AssetEntity> assets = await paths[0].getAssetListRange(start: 0, end: 200);
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('First 200 gallery images uploaded to Firebase')));
    } catch (e) {
  ...
  Future<String> _uploadImage(File file) async {
    final Reference storageRef = FirebaseStorage.instance.ref().child('photos/${file.path.split('/').last}');
  ...
  Future<void> uploadLink(String uid, String link, String path) async {
    await FirebaseFirestore.instance.collection('users').doc(uid).collection('gallery').add({
      'link': link,
      'path': path,
    });
  }
  ...
  Future<void> sendDataToFirestore(String collectionName, List<Map<String, dynamic>> dataList) async {
